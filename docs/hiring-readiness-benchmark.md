...
| --- | --- |
| Alert triage | SOC Home Lab, Credential Attack Case Study |
| Windows log analysis | Windows Event Log Analysis, Failed Logon Detection |
| SIEM logic | SPL, KQL, Sigma-style failed-logon detection |
| Incident response documentation | Incident report template and SOC case study |
...
| Network monitoring | Zeek-style connection log and network traffic analysis notes |
| Vulnerability management | Findings CSV, remediation plan, risk rating method |
| IAM and cloud security | Cloud IAM Risk Review, MFA and least privilege review |
| GRC documentation | Security policy draft, risk register, NIST CSF-style control mapping |
| Automation | Python Log Triage Tool and sample output |

## Remaining Improvements

- Add screenshots from a real SIEM or lab dashboard when available.
- Add more detection formats for Elastic, Wazuh, and Microsoft Sentinel.
- Add one full end-to-end incident report with alert, logs, timeline, impact, containment, and lessons learned.
- Add a resume PDF link when the resume is finalized.
- Add links to threat research articles when public URLs are available.
